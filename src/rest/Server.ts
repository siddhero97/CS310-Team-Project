...
export default class Server {
    ...
    public start(): Promise<boolean> {
        ...
        return new Promise(function (fulfill, reject) {
            try {
                ...
                that.rest.post("/query", that.queryDataset);
    ...
    private addDataset(req: restify.Request, res: restify.Response, next: restify.Next) {
        Log.trace("Inside addDataset of server");
        if (!this.insightFacade) {
            this.insightFacade = new InsightFacade();
        }
        const that = this;
        ...
        return that.insightFacade.addDataset(id, content, kind).then((result) => {
            Log.info(result);
            res.json(200, {result: result});
            return next();
        }).catch((e) => {
            if (e instanceof InsightError) {
                Log.error("Server::deleteDatasets InsightError - responding " + 400);
    ...
    private deleteDataset(req: restify.Request, res: restify.Response, next: restify.Next) {
        Log.trace("Inside deleteDataset of server");
        if (!this.insightFacade) {
            this.insightFacade = new InsightFacade();
        }
        const id = req.params.id;
        const that = this;
        return that.insightFacade.removeDataset(id).then((result) => {
            Log.info(result);
            res.json(200, {result: result});
            return next();
        }).catch((e) => {
            ...
            if (e instanceof NotFoundError) {
                Log.error("Server::deleteDatasets NotFoundError - responding " + 404);
                res.json(404, {error: e.toString()});
                return next();
            }
        });
    }
    ...
    private queryDataset(req: restify.Request, res: restify.Response, next: restify.Next) {
        Log.trace("Inside queryDataset of server");
        if (!this.insightFacade) {
            this.insightFacade = new InsightFacade();
        }
        return this.insightFacade.performQuery(req.body).then((result) => {
            res.json(200, {result: result});
            return next();
        }).catch((e) => {
            Log.info(e.toString());
    ...
    private getDatasets(req: restify.Request, res: restify.Response, next: restify.Next) {
        Log.trace("Inside getDatasets of server");
        if (!this.insightFacade) {
            this.insightFacade = new InsightFacade();
        }
        const that: Server = this;
        return that.insightFacade.listDatasets().then((result) => {
            Log.info(result);
            res.json(200, {result: result});
            return next();
        }).catch((e) => {
